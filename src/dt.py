...
def bowyer_watson(u_idx, delaunay_node_coords, elem_nodes, delaunay_dic_edge_triangle, most_recent_idx, triangle_most_recent_idx, delaunay_node_elems, visualize=False):
    """
    ...
    initial_bad = walk_to_point(most_recent_idx, u_idx, delaunay_node_coords, elem_nodes, delaunay_dic_edge_triangle, delaunay_node_elems ,triangle_most_recent_idx, visualize)

    # 2. Find all the others by a depth-first search in the triangulation.
    bad_elem_nodes = find_bad_elem_nodes(initial_bad, u_idx, delaunay_node_coords, elem_nodes, delaunay_dic_edge_triangle , visualize)
...
def delaunay_triangulation(cloud_node_coords, visualize=False, verbose=1):
    ...
    step_start = time()
    delaunay_node_coords = initialize_triangulation(cloud_node_coords, elem_nodes, delaunay_dic_edge_triangle, delaunay_node_elems)
    ...
    for insertion_count, point_idx in enumerate(biaised_random_ordering, 1):
        ...
        try:
            triangle_most_recent_idx, most_recent_idx = bowyer_watson(
                u_idx, delaunay_node_coords, elem_nodes, delaunay_dic_edge_triangle, most_recent_idx, triangle_most_recent_idx, delaunay_node_elems, visualize
            )
            step_time = time() - step_start
            log(f"Inserted point {insertion_count}/{total_points} in {step_time:.4f} seconds.", verbose, level=4)
            log(f"Total delaunay_node_coords: {len(delaunay_node_coords)} | Total elem_nodes: {len(elem_nodes)}", verbose, level=4)
    ...
    log(f"Final triangulation has {len(delaunay_node_coords)} delaunay_node_coords and {len(elem_nodes)} elem_nodes.", verbose, level=1)
    
    return delaunay_node_coords, elem_nodes, delaunay_dic_edge_triangle, delaunay_node_elems, (triangle_most_recent_idx, most_recent_idx)
