...
def add_triangle(u, v, w, triangles, edge_to_triangle, vertex_to_triangles):
    """
    Adds a new triangle (u, v, w) to the triangulation and updates the edge_to_triangle and vertex_to_triangles structure.

    Parameters:
    - u, v, w: Vertex indices of the new triangle.
    - triangles: List of triangles, where each triangle is a tuple of 3 vertex indices.
    - edge_to_triangle: Dictionary mapping each edge (tuple of 2 vertex indices) to one or two triangle indices.
    - vertex_to_triangles: List of triangle indices for each vertex.
    ...
    # Step 3: Update edge_to_triangle
    for edge in edges:
        if edge in edge_to_triangle:
            existing = edge_to_triangle[edge]

            if isinstance(existing, int):
                # Edge is currently shared by one triangle
                existing_triangle_idx = existing

                # Update edge_to_triangle to reflect that this edge is now shared by two triangles
                edge_to_triangle[edge] = (existing_triangle_idx, new_triangle_idx)
            ...
            edge_to_triangle[edge] = new_triangle_idx

    # Step 4: Update vertex_to_triangles
    for vertex in (u, v, w):
        vertex_to_triangles[vertex].append(new_triangle_idx)


    return new_triangle_idx

def delete_triangle(t_idx, triangles, edge_to_triangle, vertex_to_triangles):
    """
    Deletes a triangle from the triangulation and updates the edge_to_triangle structure.
    ...
    # Step 1: Remove the triangle from edge_to_triangle
    for edge in edges:
        if edge in edge_to_triangle:
            existing = edge_to_triangle[edge]
            ...
            if existing == t_idx:
                ...
                edge_to_triangle[edge] = other_triangle_idx
            elif isinstance(existing, int):
                # Edge is shared by another triangle; no action needed
                pass
            else:
                raise ValueError(f"Invalid entry in edge_to_triangle for edge {edge}.")


    # Step 2: Remove the triangle from vertex_to_triangles
    for vertex in tri:
        vertex_to_triangles[vertex].remove(t_idx)

    # Step 3: Remove the triangle by setting it to None
    triangles[t_idx] = None

def get_triangle_neighbors(triangle_idx, triangles, edge_to_triangle):
    """
...
def get_triangle_neighbors_constrained(triangle_idx, triangles, edge_to_triangle, constrained_edges_set):
    """
    Retrieves the neighboring triangles of a given triangle by inspecting shared edges,
...
def adjacent(v, w, triangles, edge_to_triangle):
    """
    ...
    if sorted_edge not in edge_to_triangle:
        # print(f"Edge {sorted_edge} not in edge_to_triangle.")
        return None  # Edge does not exist in the triangulation
    
    triangles_idx = edge_to_triangle[sorted_edge]
...
def find_containing_triangle(u_idx, triangles, edge_to_triangle, vertices):
    """
    ...
    for idx, tri in enumerate(triangles):
        # Reorder the triangle vertices so that in_circle works correctly, and for it to work, the three points need occur in counterclockwise order around the circle.
        if tri is not None:
            a, b, c = tri
...
def adjacent_2_vertex(u, triangles, edge_to_triangle, vertex_to_triangles):
    """
    Return vertices v, w such that uvw is a positively oriented triangle
    """
    adjacent_vertices = set()
    for tri_idx in vertex_to_triangles[u]:
        ...
        adjacent_vertices.add((v, w))
    
    # Convert to list or desired format
    return list(adjacent_vertices)

def get_one_triangle_of_vertex(vertex, vertex_to_triangles, triangles):
    """
    Returns one active triangle that the given vertex is part of.

    Parameters:
    - vertex (int): Index of the vertex.
    - vertex_to_triangles (List[List[int]]): Adjacency list mapping each vertex to its triangles.
    ...
    if vertex < 0 or vertex >= len(vertex_to_triangles):
    ...
    for tri_idx in vertex_to_triangles[vertex]:
...
def get_neighbor_through_edge(current_triangle_idx, edge, edge_to_triangle):
    """
    Finds the neighboring triangle across the given edge using edge_to_triangle.

    Parameters:
    - current_triangle_idx: Index of the current triangle.
    - edge: Tuple containing the two vertices that form the edge (v1, v2).
    - edge_to_triangle: Dictionary mapping each edge to one or two triangle indices.

    Returns:
    - The index of the neighboring triangle that shares the given edge, or -1 if no such neighbor exists.
    """
    # Sort the edge to match the convention used in edge_to_triangle
    sorted_edge = tuple(sorted(edge))

    triangles_sharing_edge = edge_to_triangle.get(sorted_edge, [])
    ...
    if isinstance(triangles_sharing_edge, int):
        ...
        raise ValueError("Invalid edge_to_triangle mapping.")
